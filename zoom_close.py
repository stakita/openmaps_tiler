...
def build_mosaic_image(file_map):
    ...
    x_pix_size = (xmax - xmin + 1) * tile_size
    y_pix_size = (ymax - ymin + 1) * tile_size
    print('x_pix_size', x_pix_size)
    print('y_pix_size', y_pix_size)

    full_image = Image.new('RGB', (x_pix_size, y_pix_size), 'gray')
...
def main(args):
    ...
    offset_path_t_l = list(map(lambda p: (p[0] - half_frame, p[1] - half_frame), pix_points_array))
    offset_path_b_l = list(map(lambda p: (p[0] - half_frame, p[1] + half_frame), pix_points_array))
    offset_path_t_r = list(map(lambda p: (p[0] + half_frame, p[1] - half_frame), pix_points_array))
    offset_path_b_r = list(map(lambda p: (p[0] + half_frame, p[1] + half_frame), pix_points_array))
    # pprint.pprint(offset_path_b_r)
    ...
    mosaic.save(output_file + '.raw.png')
