...
def build_mosaic_image(file_map):

    tile_size = 256

    # Calculate final image size
    xmin = xmax = ymin = ymax = None
    for tile_coord in file_map.keys():
        xtile, ytile = tile_coord
        if xmin is None or xtile < xmin:
            xmin = xtile
        if xmax is None or xtile > xmax:
            xmax = xtile
        if ymin is None or ytile < ymin:
            ymin = ytile
        if ymax is None or ytile > ymax:
            ymax = ytile
    ...
    x_pix_size = (xmax - xmin) * tile_size
    y_pix_size = (ymax - ymin) * tile_size
    print('x_pix_size', x_pix_size)
    print('y_pix_size', y_pix_size)

    full_image = Image.new('RGB', (x_pix_size, y_pix_size))

    for tile_coord, filename in file_map.items():
        print(tile_coord, filename)
        xtile, ytile = tile_coord
        tile_image = Image.open(filename)
        full_image.paste(tile_image, ((xtile - xmin) * tile_size, (ytile - ymin) * tile_size))

    return full_image, xmin, ymin
...
def main(args):
    ...
    # geo_points_array = get_ll_points_array(points_list)
    pix_points_array = get_pixel_mapped_points_array(points, zoom_factor)
    # pprint.pprint(pix_points_array)
    frame_size = 1022
    half_frame = 511
    track_path = list(map(lambda p: (p[0], p[1]), pix_points_array))
    offset_path_0_0 = list(map(lambda p: (p[0] - half_frame, p[1] - half_frame), pix_points_array))
    offset_path_0_1 = list(map(lambda p: (p[0] - half_frame, p[1] + half_frame), pix_points_array))
    offset_path_1_0 = list(map(lambda p: (p[0] + half_frame, p[1] - half_frame), pix_points_array))
    offset_path_1_1 = list(map(lambda p: (p[0] + half_frame, p[1] + half_frame), pix_points_array))
    # pprint.pprint(offset_path_1_1)
    ...
    mosaic.save('mosaic_raw.png')

    xpix_min = xtile2pix(xtile_min, zoom_factor)
    ypix_min = ytile2pix(ytile_min, zoom_factor)
    scale_factor = 1

    scaled_image_pixel_points = get_scaled_inter_image_pixel_points_array(points, zoom_factor, xpix_min, ypix_min, scale_factor)
    scaled_image_pixel_points_ts = get_scaled_inter_image_pixel_points_array(points, zoom_factor, xpix_min, ypix_min, scale_factor, with_ts=True)
